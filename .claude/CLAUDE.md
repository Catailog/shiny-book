...
- **UI 및 스타일링:**
  ...
  - **알림(Toast):** 상태 변경, 에러 등 사용자 피드백은 커스텀 컴포넌트를 새로 만들지 않고 `sonner`를 사용합니다. 단, React Hook Form의 `formState.errors`로 추적되는 필드 검증 에러는 예외로, 해당 필드 바로 아래 인라인 텍스트(`text-destructive`)로 표시하고 토스트를 쓰지 않습니다. 폼 필드 검증이 아닌 나머지(수동 검증, 서버 액션 결과, 업로드/네트워크 실패 등)는 전부 토스트로 통일합니다.
