...
- **UI 및 스타일링:**
  ...
  - **알림(Toast):** 상태 변경, 에러 등 사용자 피드백은 커스텀 컴포넌트를 새로 만들지 않고 `sonner`의 `toast.*`를 사용합니다. 단, React Hook Form의 `formState.errors`로 추적되는 필드 검증 에러는 예외로, 해당 필드 바로 아래 인라인 텍스트(`text-destructive`)로 표시하고 토스트를 쓰지 않습니다. 폼 필드 검증이 아닌 나머지(수동 검증, 서버 액션 결과, 업로드/네트워크 실패 등)는 전부 토스트로 통일합니다.
    - **새 폼/피드백을 추가할 때**는 같은 성격의 기존 형제 폼이 에러를 어디에 표시하는지(인라인 vs 토스트)부터 대조하고 그 패턴을 따릅니다. 형제 코드와 위 기준이 어긋나면 형제 코드가 아니라 위 기준을 따릅니다. 형제 파일 대조는 보안 유틸뿐 아니라 에러 표시 위치 같은 일반 UI 패턴에도 적용합니다.
    - **특정 화면 영역과 결부된 수동 검증 토스트**(주소 미선택, 사진 매수 불일치 등)는 토스트만 띄우지 않고, 해당 섹션으로 스크롤 이동시킨 뒤 `@/hooks/use-error-highlight`의 `useErrorHighlight().trigger(target)`로 Coachmark의 펄스 링 애니메이션을 1회 재생해 위치를 강조합니다. 강조용 애니메이션을 새로 만들지 않고 이 훅을 재사용합니다.
    - **자동으로 사라지면 안 되는 중요한 피드백**(사용자가 놓치면 안 되는 경고나 결과)은 `@/lib/toast`의 `toastImportant.*`(`error`/`warning`/`info`/`success`)를 씁니다. `duration: Infinity` + 닫기 버튼이라 사용자가 직접 닫아야 합니다. 남용하지 않고 대부분의 피드백은 자동 소멸하는 일반 `toast.*`로 둡니다.
