...
[버전 1.4](/resources/release-note)까지는 입금 오류가 일어나면 결제 상태가 `DONE`에서 `CANCELED`로 변경됩니다. 마찬가지로 구매자에게 다시 입금하도록 안내해야 합니다.
...
### ORDER\_PAYMENT\_STATUS\_CHANGED
...
상태가 변경된 [Payout 객체](/reference/additional#payout-객체)입니다. 사용하는 API 버전에 따라 객체 필드가 다를 수 있습니다.

### seller.changed

셀러의 상태가 `APPROVAL_REQUIRED`, `PARTIALLY_APPROVED`, `KYC_REQUIRED`, `APPROVED`로 바뀌면 웹훅이 전송됩니다. KYC 심사를 마친 셀러는 1년 또는 3년에 다시 심사를 받아야 됩니다. 그럼 셀러의 상태는 다시 `KYC_REQUIRED`로 바뀝니다.

> **경고**: **셀러 등록 직후 `APPROVAL_REQUIRED`로 진입할 때는 웹훅이 발송되지 않습니다.** 개인정보가 변경돼 `PARTIALLY_APPROVED`에서 `APPROVAL_REQUIRED`로 돌아가는 경우에만 웹훅이 발송됩니다.
...
flowchart TD
    ...
    id4 -->|개인 및 개인사업자 셀러| id5([APPROVAL_REQUIRED]):::yesWebhook
...
상태가 변경된 [Seller 객체](/reference/additional#seller-객체)입니다. 사용하는 API 버전에 따라 객체 필드가 다를 수 있습니다.

## ARS결제

### ars-reservation.changed

ARS결제 예약의 상태가 변경되면 웹훅이 전송됩니다. 결제가 완료(`DONE`)되거나 가맹점이 예약을 취소(`CANCELED`)할 때 웹훅을 받을 수 있어요.

```mermaid
---
title: ARS결제 예약 상태(status) 흐름
---
%%{
  init: {
    'themeVariables': {
...
flowchart TD
    ...
    id1([ARS 예약 생성]):::noWebhook -->|승인 성공| id2([DONE]):::yesWebhook
    id1 -->|예약 취소| id3([CANCELED]):::yesWebhook
...
```json title="예시 — 결제 완료(status=DONE)"
{
  "eventType": "ars-reservation.changed",
  "createdAt": "2026-06-16T14:53:18+09:00",
  "version": "2024-06-01",
  "eventId": "whevt_a01kv7fvhdh4729vw8hpjc5cs0d",
  "entityType": "ars-reservation",
  "entityBody": {
    "arsReservationKey": "arsrsv_a01kv7fnyy3bw5d16gpj1jffg4d",
    "reserveNumber": "1234567892",
    "requestedAt": "2026-06-16T14:50:15+09:00",
    "reservedAt": "2026-06-16T14:50:15+09:00",
    "status": "DONE",
    "canceledAt": null,
    "cancelReason": null,
    "completedAt": "2026-06-16T14:53:18+09:00",
    "mId": "tosspayments"
  }
}
```
...
웹훅 이벤트 타입입니다. ARS결제 예약 상태 변경 이벤트는 `ars-reservation.changed`입니다.
...
웹훅의 타입입니다. `ars-reservation`이 돌아옵니다.

name: entityBody
type: object

상태가 변경된 [Reservation 객체](/reference/ars#reservation-객체)입니다.
